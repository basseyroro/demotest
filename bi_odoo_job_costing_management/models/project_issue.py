...
class ProjectIssue(models.Model):
    ...
    def task(self):
        ...
        if support_ids:
            project = self.env['ir.actions.act_window']._for_xml_id('project.action_view_task')
    ...
    @api.depends('date_create', 'date_action_last', 'date_last_stage_update')
    def _compute_inactivity_days(self):
        self.inactivity_days = None
        current_datetime = fields.Datetime.from_string(fields.Datetime.now())
        for issue in self:
            dt_create_date = fields.Datetime.from_string(issue.date_create) or current_datetime
    ...
    @api.depends('date_create', 'date_closed', 'date_open')
    def _compute_day(self):
        self.day_open = None
        self.working_hours_open = None
        self.day_close = None
        self.working_hours_close = None
        for issue in self:
            ...
            if issue.date_create:
                dt_create_date = fields.Datetime.from_string(issue.date_create)
                if issue.date_open:
    ...
    def _track_template(self, tracking):
        ...
        if 'stage_id' in tracking and self.stage_id.mail_template_id:
    ...
    def message_post(self, subtype_id=False, **kwargs):
        self.ensure_one()
        mail_message = super(ProjectIssue, self).message_post(subtype_id=subtype_id, **kwargs)
        if subtype_id:
